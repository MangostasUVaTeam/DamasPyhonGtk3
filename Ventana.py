...
def selecionarMovi(casilla, posicion):
	...
	if len(mueveme) == 4:
		...
		if entradaPermitida(movimiento) == True:
			...
			c1,c2,c3,c4 = ord(movimiento[0]) - 65, int(movimiento[1])-1, ord(movimiento[2]) - 65, int(movimiento[3])-1
...
def addPosibles(valor1,valor2, x,y, lista,posibles, superX, superY):
	if (valor1 >= 0) and (valor2 >= 0):
		...
		try:
			...
			if (not tablero[valor1+x+superX][valor2+y+superY].vacia) and (tablero[valor1+(2*x+superX)][valor2+(2*y+superY)].vacia) and (tablero[valor1+x+superX][valor2+y+superY].color != turno):
...
def copiarTablero(tableroOriginal):
	...
	for y in range(8):
		for x in range(8):
			tableroNuevo[y][x] = Casilla(tableroOriginal[y][x].color,tableroOriginal[y][x].tipo, chr(65+y)+str(x+1),tableroOriginal[y][x].vacia)
